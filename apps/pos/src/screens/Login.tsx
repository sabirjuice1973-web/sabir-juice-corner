...
export function Login({ onSuccess }: { onSuccess: (user: AuthUser, accessToken: string, refreshToken: string) => void }) {
    ...
    <div className="flex h-full items-center justify-center bg-gradient-to-br from-sjc-100 via-sjc-200 to-sjc-300">
      <form onSubmit={submit} className="card w-full max-w-sm p-8 space-y-5">
        <div className="flex flex-col items-center text-center">
          ...
          <div className="text-sm text-accent-700 font-medium mt-3">POS — sign in</div>
        </div>
        ...
        <div className="text-xs text-slate-400 text-center pt-2 border-t">
          Dev login: <code>admin</code> / <code>ChangeMe!2026</code>
        </div>
      </form>
    </div>
  );
