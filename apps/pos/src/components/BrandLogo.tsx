/**
 * Sabir Juice Corner brand logo.
 *
 * Variants:
 *   • "color"   — colored juice glass (default, used in headers).        Asset: /logo.png
 *   • "mono"    — black/white line-art version (good for B&W receipts).  Asset: /logo-mono.png
 *   • "solid"   — solid black silhouette (high-contrast B&W).            Asset: /logo-solid.png
 *   • "stacked" — colored logo on top + red "SABIR 1973" wordmark below
 *                 (the full official brand mark, ideal for hero placements).
 *                 Asset: /logo-stacked.png
 *
 * If the asset file is missing, we render a recognisable inline SVG fallback so
 * the UI is never broken — useful for fresh checkouts before logos are copied in.
 *
 * Owners replace artwork by saving PNG/JPG files into `apps/{pos,admin}/public/`.
 * See `public/README-logos.md` for the full file list.
 */
...
type Props = {
  size?: number;          // box size for color/mono/solid (square). For stacked, used as height.
  variant?: Variant;
  withWordmark?: boolean; // only meaningful for the SVG fallback of the non-stacked variants
  className?: string;
};
...
export function BrandLogo({ size = 96, variant = "color", withWordmark = true, className = "" }: Props) {
  ...
  useEffect(() => {
    ...
    fetch(candidate, { method: "HEAD" })
      .then((r) => { if (r.ok) setLogoSrc(candidate); })
      .catch(() => {});
  }, [variant]);

  if (logoSrc) {
    // For stacked we let it grow taller than wide; cap by height only.
    if (variant === "stacked") {
      return (
        <img
          src={logoSrc}
          alt="Sabir Juice Corner"
  ...
  // Inline SVG fallback — only renders if the PNG file is missing
  ...
  return (
    <svg
      width={variant === "stacked" ? size * 0.8 : size}
      height={size}
      viewBox="0 0 120 140"
      xmlns="http://www.w3.org/2000/svg"
      role="img"
      aria-label="Sabir Juice Corner logo"
      className={className}
    >
